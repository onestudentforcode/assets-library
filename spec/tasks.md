...
- [ ] T056 定义 VideoSceneBatch、批次状态、来源字段、`scene_split_failed` 和新上传 API 契约
- [ ] T057 增加来源批次持久化、批次任务队列、Asset 一对多关联和数据库迁移
- [ ] T058 实现场景检测 HTTP 客户端，并在不迁移或重写 Python 核心的前提下，通过 Compose 构建和启动固定提交、仅监听本机地址的场景检测服务
- [ ] T059 实现 `/api/uploads/video-scenes` 的流式上传、批次状态查询、原片删除和外部任务清理
- [ ] T060 实现完整清单与下载字节数的 7 MiB 双重门禁、原子子素材创建、批次处理中操作锁、统一发布、整批回滚和幂等恢复
- [ ] T061 上传页默认改用分镜批次接口，并在上传状态、概览和详情中展示来源原文件名
- [ ] T062 覆盖大小边界、零解析门禁、服务异常、整批回滚、直接入库原子性、恢复幂等和旧接口兼容测试
- [ ] T063 同步环境变量、Compose、README、API 文档、OpenAPI 和快速验证说明，并记录 Scene Detection Service 源码最小改动原则
- [ ] T064 通过 lint、typecheck、test、e2e、build 和分镜服务健康检查
